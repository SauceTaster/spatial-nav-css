...
describe('focus parked on a non-navigable wrapper', () => {
  ...
  it('re-adopts the focused element when an overlay un-hides the page', async () => {
    ...
    trigger.focus()
    expect(nav.getFocused()).toBeNull()
    expect(trigger.classList.contains('spatial-focused')).toBe(false)
    expect(trigger.hasAttribute('data-spatial-focused')).toBe(false)

    page.removeAttribute('aria-hidden')
    await new Promise((r) => setTimeout(r, 0))
    expect(nav.getFocused()?.id).toBe('trigger')
    expect(document.activeElement).toBe(trigger)
    expect(trigger.classList.contains('spatial-focused')).toBe(true)
    expect(trigger.hasAttribute('data-spatial-focused')).toBe(true)
    nav.destroy()
  })
})
